 ...
 *
 * $Id: wl_cfg80211.c 315295 2012-02-16 06:55:23Z $
 */
...
#ifdef CONFIG_WIFI_SUPPORT_5GHZ
//bill.jung@lge.com - First scan 2.4 only
static int gFirstScan = false;
//static uint gInitialBand;
#endif
...
#ifdef D11AC_IOTYPES // LGE_PATCH
#define WL_CHANSPEC_CTL_SB_NONE WL_CHANSPEC_CTL_SB_LLL
#endif /* D11AC_IOTYPES */ // LGE_PATCH
	...
	struct cfg80211_scan_request *request)
{
	...
	if( gFirstScan == true)
	{
		...
		int scan_unassoc_time = 10;
		
		//wldev_ioctl(ndev, WLC_GET_BAND, &gInitialBand, sizeof(uint), false);
		/*

		if( gInitialBand == WLC_BAND_5G )
		{
			gFirstScan = false;
		}
		else
		{
		*/
			//wldev_ioctl(ndev, WLC_SET_BAND, &band, sizeof(uint), true);
			wldev_ioctl(ndev, WLC_SET_SCAN_UNASSOC_TIME, &scan_unassoc_time, sizeof(int), true);
		/*
		}
		*/
	}
