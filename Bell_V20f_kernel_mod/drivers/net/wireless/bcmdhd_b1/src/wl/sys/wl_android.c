 ...
 *
 * $Id: wl_android.c 311480 2012-01-30 10:35:59Z $
 */
...
/*LGE_PATCH_S 20120522 yangseon.so set p2p max assoc */
...
/*LGE_PATCH_E 20120522 yangseon.so set p2p max assoc */
...
int wl_android_priv_cmd(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (strnicmp(command, CMD_START, strlen(CMD_START)) == 0) {
		...
		DHD_ERROR(("%s, Received regular CMD_SETFWPATH command\n", __FUNCTION__));
	...
	if (strnicmp(command, CMD_STOP, strlen(CMD_STOP)) == 0) {
		DHD_ERROR(("%s, Received regular CMD_STOP command\n", __FUNCTION__));
...
static int wifi_add_dev(void)
{
	DHD_ERROR(("## Calling platform_driver_register\n"));
...
static void wifi_del_dev(void)
{
	DHD_ERROR(("## Unregister platform_driver_register\n"));
