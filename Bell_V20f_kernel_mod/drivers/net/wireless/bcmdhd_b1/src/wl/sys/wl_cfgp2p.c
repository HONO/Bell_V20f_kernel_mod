 ...
 *
 * $Id: wl_cfgp2p.c 311480 2012-01-30 10:35:59Z $
 *
 */
#include <typedefs.h>
#include <linuxver.h>
            ...
            const wl_event_msg_t *e, void *data)
{
	...
	if (wl_get_p2p_status(wl, LISTEN_EXPIRED) == 0) {
		...
		// LGE_CHANGE_S, moon-wifi@lge.com by freddy.jang, 2012-07-05, Added wl->p2p null check
		if (wl->p2p && timer_pending(&wl->p2p->listen_timer)) {
			spin_lock_bh(&wl->p2p->timer_lock);
			del_timer_sync(&wl->p2p->listen_timer);
			spin_unlock_bh(&wl->p2p->timer_lock);
		}
		// LGE_CHANGE_E, moon-wifi@lge.com by freddy.jang, 2012-07-05, Added wl->p2p null check
