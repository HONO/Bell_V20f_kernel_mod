 ...
 *
 * $Id: dhd_linux.c 313490 2012-02-07 23:28:53Z $
 */
...
/* LGE_CHANGE_S [jooin.woo@lge.com] 2012-12-12, mac write */
...
/* LGE_CHANGE_E [jooin.woo@lge.com] 2012-12-12, mac write */
...
#endif /* defined(CONFIG_LGE_BCM432X_PATCH) && defined(P2P_PATCH) */
...
#ifdef DHD_DEBUG
#ifndef SRCBASE
#define SRCBASE        "drivers/net/wireless/bcmdhd_b2"
#endif
#define DHD_COMPILED "\nCompiled in " SRCBASE
#else
#define DHD_COMPILED
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	ifidx = dhd_net2idx(dhd, net);
	DHD_ERROR(("%s: ifidx %d, cmd 0x%04x\n", __FUNCTION__, ifidx, cmd));
...
dhd_open(struct net_device *net)
{
	...
	{
		if( strcmp( fw_path, "/system/etc/firmware/fw_bcmdhd_mfgtest.bin") == 0 )
		{
			DHD_ERROR(("WiFi MFG Firmware is loaded. Control WiFi chip power"));
			pm_chg_vbatt_fet_on(1);
		    	mdelay(150);
		}
		else
		{
			DHD_ERROR(("WiFi Cannot Turn on when PIF Cable is attached  "));		
			//return -EINVAL;
			ret = -1;
			goto exit;
		}
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (dhd_dpc_prio >= 0) {
		...
		DHD_ERROR(("Initialize DPC thread\n"));
...
static int dhd_get_curr_etheraddr(dhd_pub_t *dhd, char* macaddr)
{
	...
	if (!ret)	{
		...
		ret = dhdcdc_query_ioctl(dhd, 0, WLC_GET_VAR, buf, sizeof(buf), action_temp);
		if (ret < 0) {
...
#if !defined(OOB_INTR_ONLY) && defined(BCMLXSDMMC)	/* LGE_patch */
	if(!dhd_download_fw_on_driverload && !g_ifup)
	{
		int ret;
		ret = dhd_dev_reset(net, TRUE);
		sdioh_stop(NULL);
		if( ret < 0 )
			DHD_ERROR(("dhd_dev_reset error. return :%d \n",ret));
	}
#endif

#if 1 && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27))
	if (ifidx == 0) {
	DHD_ERROR(("release dhd_registration_sem \n"));
		up(&dhd_registration_sem);
	}
#endif 
	return 0;
