...
static const char *atcmd_ap[] = {
    ...
    "%PTNCLR", "%LGANDROID", "%LGATSERVICE", "%LGPWD", "%PNUM", "%OSPPWDINIT", "%USB",

#if defined(CONFIG_MACH_LGE_I_BOARD_LGU)
    "%MTV", "%NFC", "$LGAPP", "%SIMPWDINIT", "%SYNCTYPE",
#elif defined(CONFIG_MACH_LGE_I_BOARD_VZW)
    ...
    "%MTV", "%IMA", "%IDM", "%EXTIDM", "%FELICATX", "%SWITCH", "%FREQCAL", "%RFIDCK", "%RFREGCAL", "%SWTABLE", "%CFREQ",
#endif
...
// P2BT kwanseok.kim 20120404 - VBatt for Bluetooth; P930 HW dependancy [START] 
		...
		// 20120522 : for CANADA (add TLS, BELL)
...
void atcmd_disconnect(void)
{
    //[START]park.gyutae@lge.com  usb cable unplug during pcsync processing.
    if (atcmd_info.op_mode == ATCMD_OP_MODE_PCSYNC)
    {
        pr_info("pcsync disconnect");
        wake_up_interruptible(&atcmd_read_wait);
    }
    //[END]park.gyutae@lge.com usb cable unplug during pcsync processing.
}
