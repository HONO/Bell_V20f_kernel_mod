/* lge/lge_board/i_bell/board_i_bell_snd.c
 *
...
//Sound Patch from GB to ICS[Start] - 3
#define CONFIG_LGE_AUDIO_USE_AUXMIC_FOR_SPEAKER_MODE
//Sound Patch from GB to ICS[End] - 3
//Sound Patch from GB to ICS[Start] - 7
#define CONFIG_LGE_AUDIO_SPEAKER_MONO
//#define CONFIG_LGE_AUDIO_SPEAKER_MONO_PLAYBACK
//Sound Patch from GB to ICS[End] - 7
//Sound Patch from GB to ICS[Start] - 9
#define CONFIG_LGE_AUDIO_HEADSET_MONO
//Sound Patch from GB to ICS[End] - 9
...
//Sound Patch from GB to ICS[Start] - 1
#include "timpani_profile_8x60_i_bell.h"
//Sound Patch from GB to ICS[End] - 1
...
//Sound Patch from GB to ICS[Start] - 2
...
//Sound Patch from GB to ICS[End] - 2
enum {
    RCV_RX1_L_GAIN,
    RCV_GAIN,
    RCV_FE_GAIN,
...
//Sound Patch from GB to ICS[End] - 7
    ...
    HPH_RX1_L_GAIN,
//Sound Patch from GB to ICS[Start] - 9
#ifdef CONFIG_LGE_AUDIO_HEADSET_MONO
#else
    HPH_RX1_R_GAIN,
#endif
//Sound Patch from GB to ICS[End] - 9
    ...
    P_SPK_RX1_L_GAIN,
//Sound Patch from GB to ICS[Start] - 7
#ifdef CONFIG_LGE_AUDIO_SPEAKER_MONO_PLAYBACK
#else
    P_SPK_RX1_R_GAIN,
#endif       
//Sound Patch from GB to ICS[End] - 7
    ...
    P_HPH_L_GAIN,
    P_HPH_R_GAIN,
//Sound Patch from GB to ICS[Start] - 6
    RECORD_FE_GAIN,
    RECORD_TX1_GAIN,
//Sound Patch from GB to ICS[Start] - 10
    RECORD_VM_FE_GAIN,
    RECORD_VM_TX1_GAIN,
//Sound Patch from GB to ICS[End] - 10
    D_MAIN_FE1_GAIN,
    D_MAIN_TX1_L_GAIN,
    D_SUB_FE2_GAIN,
    D_SUB_TX1_R_GAIN,
//Sound Patch from GB to ICS[End] - 6
    QTR8615L_CAL_DB_MAX
};
//Sound Patch from GB to ICS[Start] - 2
...
//Sound Patch from GB to ICS[Start] - 7
  ...
  {0x01,    0x000B},
  {0x02,    0x6020},
    {0x1A,    0x0102},//IN2A vol
  {0x36,    0x0004},
  {0x03,    0x0008},
  {0x22,    0x0000},
  {0x03,    0x0108},
    {0x25,    0x0178},//boost vol
    {0x26, 0x0133},   //vol
  {0x17,    0x0002},
  {0x01,    0x100B},
#else
  ...
  {0x17, 0x0001}, // IN2_CLAMP 0bit 0 : clamp de-activate 1: clamp activated  
    {0x1A, 0x0103}, // IN2A -6dB(00}, -3.5dB(01}, 0dB(02}, 3.5dB(03}, 6dB(04}, 12dB(05}, 18dB(06}, 18dB(07)  
  {0x1B, 0x0103}, // IN2B -6dB(00}, -3.5dB(01}, 0dB(02}, 3.5dB(03}, 6dB(04}, 12dB(05}, 18dB(06}, 18dB(07)
  {0x36, 0x0005},
  {0x03, 0x0008},
  {0x22, 0x0000}, // 0dB
  {0x03, 0x0108},  
...
static wmCodecCmd seq_lin_to_headset[] = 
{
	...
	{0x01, 0x000B},
	{0x02, 0x6080},
	{0x16, 0x0003}, // IN1_CLAMP 0bit 0 : clamp de-activate 1: clamp activated
...
{
//Sound Patch from GB to ICS[Start] - 5
	int rc=0;
//Sound Patch from GB to ICS[End] - 5
	pr_debug("%s\n", __func__);
...
//Sound Patch from GB to ICS[Start] - 3
//Sound Patch from GB to ICS[Start] - 4
...
//Sound Patch from GB to ICS[End] - 4
...
//Sound Patch from GB to ICS[Start] - 8
...
//Sound Patch from GB to ICS[End] - 8
...
//Sound Patch from GB to ICS[Start] - 7
    ...
    {ispkr_stereo_48KHz_osr256_actions,             28}, // SPK_L_GAIN 0xE0
    {ispkr_stereo_48KHz_osr256_actions,             29}, // SPK_R_GAIN 0xE1
#else
    {ispkr_stereo_48KHz_osr256_actions,              6}, // SPK_RX1_L_GAIN 0x84
...
//Sound Patch from GB to ICS[Start] - 9
	...
	{ihs_stereo_48KHz_osr256_actions,				26}, // HPH_L_GAIN
	{ihs_stereo_48KHz_osr256_actions,				27}, // HPH_R_GAIN
#else
    {ihs_stereo_48KHz_osr256_actions,                6}, // HPH_RX1_L_GAIN
...
//Sound Patch from GB to ICS[Start] - 7
    ...
    {ispkr_stereo_playback_48KHz_osr256_actions,    28}, // P_SPK_L_GAIN
    {ispkr_stereo_playback_48KHz_osr256_actions,    29}, // P_SPK_R_GAIN
#else
    {ispkr_stereo_playback_48KHz_osr256_actions,     6}, // P_SPK_RX1_L_GAIN
...
#ifdef CONFIG_LGE_AUDIO_SPEAKER_MONO
      case SPK_IN2_VOL :
          seq_lin_to_spkout[3].wmdata = cal_db;
          break;

      case SPK_BOOST :
          seq_lin_to_spkout[8].wmdata = cal_db;
          break;    

      case SPK_OUT_VOL :
          seq_lin_to_spkout[9].wmdata = cal_db;
          break;
...
#ifdef CONFIG_LGE_AUDIO_HEADSET_MONO
		case HPH_IN1_VOL :
			seq_lin_to_headset[4].wmdata = cal_db;
			break;

		case HPH_OUT_VOL :
			seq_lin_to_headset[10].wmdata = seq_lin_to_headset[11].wmdata = cal_db;
			break;
#else
        case HPH_IN1_VOL :
...
#ifdef CONFIG_LGE_AUDIO_SPEAKER_MONO_PLAYBACK
        case P_SPK_IN2_VOL :
            seq_lin_to_spkout_playback[3].wmdata = cal_db;
            break;

        case P_SPK_BOOST :
            seq_lin_to_spkout_playback[8].wmdata = cal_db;
            break;    

        case P_SPK_OUT_VOL :
            seq_lin_to_spkout_playback[9].wmdata = cal_db;
            break;
...
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
//Sound Patch from GB to ICS[Start] - 2
    const char* AudioSubsystemPath  = "/data/wm9093_i_bell.txt";
    const char* AudioSubsystemPath2 = "/etc/wm9093_i_bell.txt";
//Sound Patch from GB to ICS[End] - 2
#else
...
int get_wm9093_cal_db(char *data_array, int *data_len)
{
    ...
    /* Header */
    sprintf(str, "WM9093 for %s (%s)", "P930", __DATE__);
...
#ifdef CONFIG_LGE_AUDIO_SPEAKER_MONO
    /* SPK_IN2_VOL */
    sprintf(str, wm9093_cfg_item[cnt++], seq_lin_to_spkout[3].wmdata);
    if(buff_len + strlen(str) + 4 >= ARRAY_SIZE(str)) goto END_OF_CODES;
    memcpy(&(data_array[buff_len]), str, strlen(str));
    buff_len +=  strlen(str);
    data_array[buff_len++] = '\n';
    ...
    /* SPK_BOOST */
    sprintf(str, wm9093_cfg_item[cnt++], seq_lin_to_spkout[8].wmdata);
    if(buff_len + strlen(str) + 4 >= ARRAY_SIZE(str)) goto END_OF_CODES;
    memcpy(&(data_array[buff_len]), str, strlen(str));
    buff_len +=  strlen(str);
    data_array[buff_len++] = '\n';                
    ...
    /* SPK_OUT_VOL */
    sprintf(str, wm9093_cfg_item[cnt++], seq_lin_to_spkout[9].wmdata);
    if(buff_len + strlen(str) + 4 >= ARRAY_SIZE(str)) goto END_OF_CODES;
    memcpy(&(data_array[buff_len]), str, strlen(str));
    buff_len +=  strlen(str);
    data_array[buff_len++] = '\n';      
...
/* HPH_IN1_VOL */
	...
	/* HPH_OUT_VOL */
	sprintf(str, wm9093_cfg_item[cnt++], seq_lin_to_headset[10].wmdata);
	if(buff_len + strlen(str) + 4 >= ARRAY_SIZE(str)) goto END_OF_CODES;
	memcpy(&(data_array[buff_len]), str, strlen(str));
	buff_len +=  strlen(str);
	data_array[buff_len++] = '\n'; 
...
#ifdef CONFIG_LGE_AUDIO_SPEAKER_MONO_PLAYBACK
    /* P_SPK_IN2_VOL */
    sprintf(str, wm9093_cfg_item[cnt++], seq_lin_to_spkout_playback[3].wmdata);
    if(buff_len + strlen(str) + 4 >= ARRAY_SIZE(str)) goto END_OF_CODES;
    memcpy(&(data_array[buff_len]), str, strlen(str));
    buff_len +=  strlen(str);
    data_array[buff_len++] = '\n';
    ...
    /* P_SPK_BOOST */
    sprintf(str, wm9093_cfg_item[cnt++], seq_lin_to_spkout_playback[8].wmdata);
    if(buff_len + strlen(str) + 4 >= ARRAY_SIZE(str)) goto END_OF_CODES;
    memcpy(&(data_array[buff_len]), str, strlen(str));
    buff_len +=  strlen(str);
    data_array[buff_len++] = '\n';                
    ...
    /* P_SPK_OUT_VOL */
    sprintf(str, wm9093_cfg_item[cnt++], seq_lin_to_spkout_playback[9].wmdata);
    if(buff_len + strlen(str) + 4 >= ARRAY_SIZE(str)) goto END_OF_CODES;
    memcpy(&(data_array[buff_len]), str, strlen(str));
    buff_len +=  strlen(str);
    data_array[buff_len++] = '\n';             
...
{
//Sound Patch from GB to ICS[Start] - 2
    const char* QTR8615LPath  = "/data/qtr8615l_i_bell.txt";
    const char* QTR8615LPath2 = "/etc/qtr8615l_i_bell.txt";
//Sound Patch from GB to ICS[End] - 2
    struct file *filp;
    ssize_t read_size = 0;
    char data_array[1000];
...
int get_qtr8615l_cal_db(char *data_array, int *data_len)
{
    ...
    /* Header */
    sprintf(str, "QTR8615L for %s (%s)", "P930", __DATE__);    
    if(buff_len + strlen(str) + 4 >= ARRAY_SIZE(str)) goto END_OF_CODES;
    memcpy(&(data_array[buff_len]), str, strlen(str));
    buff_len +=  strlen(str);
    data_array[buff_len++] = '\n';
...
//Sound Patch from GB to ICS[Start] - 4
	.key_code = KEY_MEDIA, // KEY_MEDIA // just for testing
//Sound Patch from GB to ICS[End] - 4
