/* lge/lge_board/j_tls/board_j_tls_snd.c
 *
...
#include "timpani_profile_8x60_j_tls.h"
...
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
//Sound Patch from GB to ICS[Start] - 2
    const char* AudioSubsystemPath  = "/data/wm9093_j_tls.txt";
    const char* AudioSubsystemPath2 = "/etc/wm9093_j_tls.txt";
//Sound Patch from GB to ICS[End] - 2
#else
...
int get_wm9093_cal_db(char *data_array, int *data_len)
{
    ...
    /* Header */
    sprintf(str, "WM9093 for %s (%s)", "P935", __DATE__);
...
{
//Sound Patch from GB to ICS[Start] - 2
    const char* QTR8615LPath  = "/data/qtr8615l_j_tls.txt";
    const char* QTR8615LPath2 = "/etc/qtr8615l_j_tls.txt";
//Sound Patch from GB to ICS[End] - 2
    struct file *filp;
    ssize_t read_size = 0;
    char data_array[1000];
...
int get_qtr8615l_cal_db(char *data_array, int *data_len)
{
    ...
    /* Header */
    sprintf(str, "QTR8615L for %s (%s)", "P935", __DATE__);    
    if(buff_len + strlen(str) + 4 >= ARRAY_SIZE(str)) goto END_OF_CODES;
    memcpy(&(data_array[buff_len]), str, strlen(str));
    buff_len +=  strlen(str);
    data_array[buff_len++] = '\n';
